...
create_lags <- function(df, lag = 14, date = FALSE) {
  ...
  # if (date == TRUE) {
  #   prefix_date <- paste0("Lag", prefix, "_holiday")
  #   df <- df %>% 
  #     mutate(!!prefix_date := df$date + days(lag))
  # }
...
The line plot reveals that ridership has increased continuously from 2001 to 2016. Ridership during the week increase from Jan to Oct then plummet each year. The weekend data fis much more heterogeneous.
